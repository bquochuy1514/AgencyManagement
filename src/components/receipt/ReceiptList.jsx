...
import { getImportReceiptById } from '../../services/receiptService';
import ImportDetailForm from '../receipt/ImportDetailForm';

const ReceiptList = ({ receipts, type }) => {
  ...
  const handleDelete = (receiptId) => {
    // Logic xóa phiếu (cần API xóa)
    console.log(`Xóa phiếu với ID: ${receiptId}`);
  };

  const handleShowDetail = async (receiptId) => {
    try {
    ...
    <div>
        ...
        <table className="min-w-full divide-y divide-gray-700">
          <thead className="bg-[#2a3b4c] text-gray-300">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">
                {type === 'import' ? 'Tổng giá' : 'Tổng tiền'}
              </th>
          ...
          <tbody className="divide-y divide-gray-700 text-gray-200">
            {currentItems.map((receipt) => (
              <tr key={receipt.importReceiptID} className="border-t border-gray-700 text-gray-200">
                ...
                <td className="px-4 py-3 flex space-x-2">
                  <button
                    onClick={() => handleShowDetail(receipt.importReceiptID)}
                  ...
                  <button
                    onClick={() => handleDelete(receipt.importReceiptID)}
      ...
      {selectedReceiptId && (
        <div className="p-6 bg-[#2a3b4c] rounded-lg mt-6">
          ...
          <ImportDetailForm receiptId={selectedReceiptId} receiptDetails={receiptDetails} onClose={() => setSelectedReceiptId(null)} />
        </div>
      )}
    </div>
  );
