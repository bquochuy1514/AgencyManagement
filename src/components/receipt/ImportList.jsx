    ...
    <div className="bg-gray-800 rounded-lg shadow-lg p-4">
      <table className="w-full text-left">
        ...
        <tbody>
          {currentItems.map((receipt) => (
            <tr
              key={receipt.importReceiptID}
              className="border-b border-gray-700 hover:bg-gray-700"
            >
              <td className="py-3 px-4">{receipt.importReceiptID}</td>
              <td className="py-3 px-4">{new Date(receipt.dateReceipt).toLocaleDateString()}</td>
              <td className="py-3 px-4">{new Intl.NumberFormat("vi-VN").format(receipt.totalPrice)} đ</td>
